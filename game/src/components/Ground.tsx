...
import { rivalTeamNameState, rivalTeamScoreState } from "../recoil/atoms";
...
const Ground: React.FC = () => {
  ...
  const [rivalTeamName, setRivalTeamName] = useRecoilState(rivalTeamNameState);
  const [awayTeamScore, setAwayTeamScore] = useRecoilState(rivalTeamScoreState);
  ...
  },[rivalTeamName, awayTeamScore, teamReady ]);
                ...
                <div className="game_pad_inner">
                  ...
                  {gameStatus === 2 || gameStatus === 3 ? (
                    <div className="game_pad_num_line">
                      ...
                      {myNumArray.length === 3 && teamReady[1] !== undefined ? (
                        <button
                          className="game_pad_num_line_info_register"
                          onClick={() =>
                            determineMyOutNum.length === 3 &&
                            teamOriginId === gameRound.toString()
                              ? determineAttackNum()
                              : alert("본인의 차례가 아니입니다.")
                          }
                        >
  ...
  return (
    <div className="game_board">
      ...
      <div className="game_board_detail">
        ...
        <GameScore homeTeamScore={homeTeamScore} awayTeamScore={awayTeamScore} />
...
//react-query 라이브러리로 처리할 예정
const ShowRound = ({gameRound} :any) =>{
  return(
...
//react-query 라이브러리로 처리할 예정
const GameScore = ({homeTeamScore, awayTeamScore}:any) => {
